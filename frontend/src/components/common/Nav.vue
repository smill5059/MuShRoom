<template>
  <div>
    <v-app-bar app color="amber darken-1" elevate-on-scroll>
      <v-app-bar-nav-icon>
        <v-img src="@/assets/logo.png" alt="" width="30" height="30" />
      </v-app-bar-nav-icon>
        ...
        <v-list>
          <v-list-item v-for="(item, i) in category" :key="i">
            ...
            >
              ...
              <v-list>
                <v-list-item v-for="(item, i) in instruments" :key="i">
                  <v-list-item-title>{{ item.title }}</v-list-item-title>
                </v-list-item>
              </v-list>
            </v-menu>
      ...
      <v-btn depressed dark :to="{ name: 'Login' }" color="amber darken-1">
        로그인
      </v-btn>
      <v-btn
        depressed
